...
export default function BriefingFeed({ alerts, meta, partnerA, partnerB, onDismiss, onSnooze, onResolve, onChat, onTackle, onAcceptSuggestion, onAddToOrders, sidebar = false }) {
  return (
          ...
          <div className="space-y-2">
            {alerts.map((a) => (
              <AlertCard key={a.id} alert={a} partnerA={partnerA} partnerB={partnerB}
                ...
                onTackle={onTackle} onAcceptSuggestion={onAcceptSuggestion} onAddToOrders={onAddToOrders} />
            ))}
          </div>
        )}
      </div>
